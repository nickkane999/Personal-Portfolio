...
export async function getAnswer(question: string, sampleFile): Promise<string> {
  const openAIFiles = new OpenAIFiles();
  const file = await openAIFiles.createFile({
    file: fs.createReadStream("C:\\Users\\nickk\\Music\\Job Portfolio\\Personal-Site\\main\\data\\documents\\DocumentationAgent\\Nicholas Kane -Resume.docx"),
    purpose: "assistants",
  });

  const agent = await OpenAIAssistantRunnable.createAssistant({
    model: "gpt-3.5-turbo-0125",
    instructions: "Answer questions about the person's resume",
    name: "Resume agent",
  ...
  let max_tokens = 200;
  const answer: any = await agent.invoke({
